...
import {
  buildWizardCreatedActivity,
  FLEET_ROLLOUT_DEMO_ACTIVITIES,
  matchesRunChannelFilter,
  runChannelLabel,
  type FleetRolloutActivity,
  type FleetRolloutActivityStatus,
  type FleetRolloutSourceFilter,
} from "./fleetRolloutDemoData";
...
type Activity = FleetRolloutActivity;
type ActivityStatus = FleetRolloutActivityStatus;
...
const suggestedFilters = [
  ...
  { label: "env=prod", count: 3 },
  { label: "OpenShift cluster update", count: 2 },
  { label: "CVE patch", count: 2 },
  { label: "Failed", count: 1 },
  { label: "Argo CD", count: 1 },
];
...
export function ActivityStreamScreen({
  ...
  const [sourceFilter, setSourceFilter] =
    useState<FleetRolloutSourceFilter>("all");

  const [activities, setActivities] = useState<Activity[]>(() => [
    ...(executionPolicy
      ? [buildWizardCreatedActivity(executionPolicy)]
      : []),
    ...FLEET_ROLLOUT_DEMO_ACTIVITIES,
  ]);
  ...
  const handleRowClick = (activity: Activity) => {
    if (activity.drilldownAvailable) {
      navigate(`${deploymentCopy.fleetRollout.basePath}/${activity.id}`, {
  ...
  const tabFilteredActivities = useMemo(() => {
    ...
    if (sourceFilter !== "all") {
      rows = rows.filter((a) => matchesRunChannelFilter(a, sourceFilter));
    }
    return rows;
      ...
      <header className="pb-2">
        <div className="flex flex-col items-end gap-3 min-[400px]:flex-row min-[400px]:items-start min-[400px]:justify-between min-[400px]:gap-4">
          <div className="min-w-0 w-full min-[400px]:w-auto self-start min-[400px]:self-auto text-left">
            ...
            <TinyText muted className="mt-1.5 !block max-w-3xl leading-snug">
              {deploymentCopy.fleetRollout.pageSubtitle}
            </TinyText>
          </div>
        ...
        >
            ...
            <div className="flex flex-wrap items-center gap-1 min-w-0">
              {(
                [
                  {
                    id: "all" as const,
                    label: deploymentCopy.fleetRollout.sourceFilterAll,
                    title: deploymentCopy.fleetRollout.sourceFilterAllTitle,
                  },
                  {
                    id: "console" as const,
                    label: deploymentCopy.fleetRollout.sourceFilterConsole,
                    title: deploymentCopy.fleetRollout.sourceFilterConsoleTitle,
                  },
                  {
                    id: "git" as const,
                    label: deploymentCopy.fleetRollout.sourceFilterGit,
                    title: deploymentCopy.fleetRollout.sourceFilterGitTitle,
                  },
                  {
                    id: "argocd" as const,
                    label: deploymentCopy.fleetRollout.sourceFilterArgocd,
                    title: deploymentCopy.fleetRollout.sourceFilterArgocdTitle,
                  },
                  {
                    id: "aap" as const,
                    label: deploymentCopy.fleetRollout.sourceFilterAap,
                    title: deploymentCopy.fleetRollout.sourceFilterAapTitle,
                  },
                ] as const
              ).map(({ id, label, title }) => (
                <button
          ...
          <div className="flex min-w-0 flex-wrap items-center gap-x-1.5 gap-y-1.5 text-[11px] leading-none">
            <span
              ...
              title="Narrow the list the same way as the table Status column (one chip at a time; Phased = multi-step rollouts)."
            >
              Deployment status
            </span>
            ...
            >
              ...
              >
                ...
                <button
                  ...
                  title={deploymentCopy.fleetRollout.sourceFilterArgocdTitle}
                  onClick={() =>
                    setSourceFilter((f) => (f === "argocd" ? "all" : "argocd"))
                  }
                  className="rounded px-0.5 text-left"
                  style={{
                    fontWeight: "var(--font-weight-medium)",
                ...
                >
                  {deploymentInsights.gitOps.anyAttention
                    ? `Argo CD · ${deploymentInsights.gitOps.totalRepos - deploymentInsights.gitOps.syncedRepoCount} repos need attention`
                    : "Argo CD · all synced"}
                </button>
              </div>
              <TinyText
            ...
            >
              ...
              >
                <div className="flex min-w-0 flex-wrap items-center gap-2">
                  ...
                  >
                    {deploymentCopy.fleetRollout.connectedSystemsTitle}
                  </TinyText>
                  <Badge variant="default" className="text-[9px] px-1.5 py-0">
                    Demo
                  </Badge>
                  <TinyText muted className="text-[10px] max-w-sm">
                    {deploymentCopy.fleetRollout.connectedSystemsHint}
                  </TinyText>
                </div>
                <Badge
                  variant={
          ...
          <tbody>
            {sortedActivities.length === 0 ? (
              ...
              >
                ...
                <td className="px-4 py-3">
                  <div className="min-w-0">
                    <TinyText muted>{runChannelLabel(activity.runChannel)}</TinyText>
                    {activity.externalRef ? (
                      <TinyText
                        muted
                        className="!block truncate text-[10px] leading-snug"
                        title={activity.externalRef.label}
                      >
                        {activity.externalRef.label}
                      </TinyText>
                    ) : null}
                  </div>
                </td>
                      ...
                      {activity.drilldownAvailable && (
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            navigate(
                              `${deploymentCopy.fleetRollout.basePath}/${activity.id}`,
                            );
                          }}
                        >
                          [View drilldown]
