...
function buildWizardSteps(): WizardStepDef[] {
  const { steps: s } = deploymentCopy;
  const h = s.hints;

  return [
    { number: 1, label: s.plan, hint: h.plan, contentId: 1 },
    { number: 2, label: s.rollout, hint: h.rollout, contentId: 2 },
    {
      number: 3,
    ...
    { number: 4, label: s.reviewCreate, hint: h.review, contentId: 4 },
  ];
}
...
export function DeploymentWizard({
  ...
  const [entryMode, setEntryMode] = useState(entryModeProp);
  const steps = useMemo(() => buildWizardSteps(), []);

  useEffect(() => {
    setEntryMode(entryModeProp);
  }, [entryModeProp]);
  ...
  const hasValidPlacement = useMemo(
    () => formDataHasValidPlacement(formData),
    [
  ...
  const planStepComplete = hasPrimaryAction && hasValidPlacement;
  ...
  const canGoToStep = useCallback(
    (stepNumber: number) => {
      ...
      return planStepComplete;
    },
    [planStepComplete, totalSteps],
  );
  ...
  const canAdvanceFromCurrentStep = useMemo(() => {
    ...
    return true;
  }, [currentStep, planStepComplete, steps, totalSteps]);
...
function StepPlanContent({
  ...
  launchTab: DeploymentTabId;
  entryMode: WizardEntryMode;
  onEntryModeChange: (mode: WizardEntryMode) => void;
}) {
  const actionSection = (
    <section className="space-y-4">
      <LabelText className="!mb-0">{deploymentCopy.steps.action}</LabelText>
      <Step1Content
        formData={formData}
        setFormData={setFormData}
        launchTab={launchTab}
  ...
  const placementSection = (
    <section className="space-y-4 border-t border-border/80 pt-8">
      <LabelText className="!mb-0">{deploymentCopy.steps.placement}</LabelText>
      <Step2Content
        formData={formData}
        setFormData={setFormData}
        entryMode={entryMode}
  ...
  const placementFirstPlacementSection = (
    <section className="space-y-4">
      <LabelText className="!mb-0">{deploymentCopy.steps.placement}</LabelText>
      <Step2Content
        formData={formData}
  ...
  const placementFirstActionSection = (
    <section className="space-y-4 border-t border-border/80 pt-8">
      <LabelText className="!mb-0">{deploymentCopy.steps.action}</LabelText>
      <Step1Content
        formData={formData}
  ...
  return (
    <div className="space-y-8">
      ...
      <section>
        <span className={planStepSectionLabelClass}>
        ...
        <ToggleGroup
          ...
          onValueChange={(v) => {
            if (v) {
              onEntryModeChange(v as WizardEntryMode);
            }
          }}
          variant="outline"
        ...
        <TinyText muted className="mt-2 !block leading-snug">
          {deploymentCopy.steps.hints.plan}
        </TinyText>
      </section>
      ...
      {entryMode === "action-first" ? (
        ...
        <>
          {placementFirstPlacementSection}
          {placementFirstActionSection}
        </>
      )}
    </div>
  );
...
function formDataHasValidPlacement(fd: {
  fleetSelection: string;
  labelSelector?: string;
  selectedClusters?: string[];
}): boolean {
  if (fd.fleetSelection === "searchable") {
    return (fd.selectedClusters?.length ?? 0) > 0;
  }
  const selector = (fd.labelSelector || "").trim();
  if (!selector) {
    return false;
  }
  return getClustersMatchingPlacement(fd).length > 0;
}
...
function Step2Content({
  ...
  return (
    <div className="space-y-8">
      {!suppressBanner && <AiPlanPrefillBanner formData={formData} />}
